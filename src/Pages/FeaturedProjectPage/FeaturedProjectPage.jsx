    ...
    <div className="projects">
      <div className="project-wrapper">
        ...
        <div className="project-descriptions">
          <ul>
            {project.language && (
              <li className="project-desc">{project.language}</li>
            )}
            {project.desc1 && <li className="project-desc">{project.desc1}</li>}
            {project.desc2 && (
              <li className="project-desc">{project?.desc2}</li>
            )}
          </ul>
        </div>
        <div className="project-desc-images">
          {[project?.img1, project?.img2, project?.img3, project?.img4] //This creates an array with four possible images from the project object.
            ...
            .map(
              (
                img,
                index //This loops through the filtered images and creates an <img> tag for each one.
              ) => (
                <img
                  key={index}
                  src={img}
                  alt={`Project Image ${index + 1}`}
                  className="project-desc-img"
                />
              )
            )}
        </div>
